...
//deine my variables and constants
const char WALL = '#';
const char EMPTY = ' ';
...
void bfs(vector<vector<char>> &maze,vector<vector<bool>> &visited,vector<vector<pair<int,int>>> &parent,int height, int width){
    ...
    cout << "resolver por BFS" << endl;
    system("pause");
    print(maze,height,width,path);
    cout << "el camino mas corto esta resaltado " << endl;
...
int main(){
    ...
    cin >> height;
    cout << "ingrese ancho para el laberinto: ";
    cin >> width;
    ...
    auto durationGen = duration_cast<milliseconds>(endGen - startGen).count();
    vector<pair<int,int>> empty_path;
    print(maze,height,width,empty_path);
    cout << "Tiempo de generación: " << durationGen << " ms" << endl;

    auto startSolve = high_resolution_clock::now();
    bfs(maze,visited,parent,height,width);
    auto endSolve = high_resolution_clock::now();
    auto durationSolve = duration_cast<milliseconds>(endSolve - startSolve).count();
    cout << "Tiempo de Resolucion: " << durationSolve << " ms" << endl;
